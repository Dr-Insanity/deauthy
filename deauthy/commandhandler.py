...
It won't be me.{end}""")
        ...
        def d_discover():
            ...
            with Halo(f"{cyan+bold+underline}Monitoring networks nearby...") as spinner:
                try:
                    out = check_output(["airodump-ng", iface, "-w", "discovered_targets", "-o", "pcap"], stdin=DEVNULL, stderr=STDOUT)
                except KeyboardInterrupt:
                    spinner.succeed(f"{light_green+bold+underline}CTRL + C pressed! Stopping monitoring.")

            out = run('tshark -Y wlan.fc.type_subtype==0x08 -e wlan.ssid -e wlan.ds.current_channel -e wlan.addr -T json -r discovered_targets-01.cap > discovered_targets.json', shell=True, stdin=DEVNULL, stderr=STDOUT)
