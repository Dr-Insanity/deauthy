...
It won't be me.{end}""")
        ...
        def d_discover():
            ...
            from deauthy.terminal import Terminal
            from deauthy.functs import get_var, mod_config, Functs
            iface = get_var('interface')
            if iface is None or iface not in [ifc[1] for ifc in if_nameindex()]:
                Terminal.inform(f"{red}{bold}Tell me what interface I should be using with the '!interface' command")
                return
            ...
            with open("discovered_targets.json", "r") as jsonfile:
                ...
                if isinstance(selected_networks, str):
                    ...
                    print(f"{light_green+bold}I proudly inform you that the selected networks were saved successfully")
        ...
        def d_start():
            ...
            if do_bssid is None and do_essid is None:
                ...
                with Halo(f"{white+bold}[{red+bold}!{white+bold}] {red+bold+underline}Deauthenticating {get_var('target_mac')} ...") as spinner:
                    Functs.BSSID_METHOD.deauth(bssids, spinner)
